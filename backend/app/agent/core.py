...
INLINE_SKILL_GUIDANCE_PROMPT = """创作流程：
- 不开启本地 skill catalog 时，以下规则直接作为基础行为约束；不要声称启用了某个 skill，也不要要求用户去读取额外说明文件。
- 面对创作、规划、角色、审稿和保存任务，先判断方向、范围、风格、上下文来源、目标文件和覆盖意图是否清楚；不清楚时简短复述理解，给出 1-3 个可选方向，并只追问会影响结果的关键问题。
- 主 Agent 负责面向用户确认目标、约束和验收标准，整理成清晰任务单后委派给子 Agent；不要直接产出完整正文、大纲、人设、世界观、审稿意见、改写稿或执行文件修改。
- 子 Agent 负责具体执行。任务信息不足时，返回缺口清单、可继续完成的部分和建议主 Agent 追问的问题，不要向用户等待输入。
- 正式写作、整理、改写、审稿或保存前，先读取小说根目录或任务单指定目录下的 `progress.md`；文件不存在时只做轻量目录检查并创建初始总结。生成、修改或保存后，同步更新进度、影响文件、待确认问题和下一步。
- 章节任务要明确第几章、视角、篇幅、承接内容、核心冲突和结尾钩子；大纲任务要落到主线目标、阶段压力、关键转折和章节可执行性；角色任务要写清外在目标、内在缺口、能力边界和关系张力；审稿任务必须基于已读正文指出位置、原因和修改方向。
- 文件路径使用当前 sandbox 相对路径，不要加 `novels/` 前缀。保存内容优先使用 Markdown；调用 `write_file` 或 `edit_file` 时必须提供完整正文，不写摘要、说明或占位内容。
- 覆盖、重写或并入已有文件前先读取原文件，并确认用户允许覆盖或修改。不要把一次性需求、闲聊、敏感信息、API key 或账号信息写入长期流程、记忆或进度文件。"""
BASE_SYSTEM_PROMPT_WITH_INLINE_SKILLS = BASE_SYSTEM_PROMPT.replace(
    """技能使用：
- 系统会提供可用 skill 的名称、说明、触发时机和说明文件路径。
- 当用户请求符合某个 skill 的触发时机时，你应主动参考该 skill；如果需要完整流程、格式或安全规则，可以读取对应的 skills/*.md 说明文件。
- 不要只因为关键词相似就机械套用 skill；应根据用户的真实任务判断是否需要使用。
- 多个 skill 都相关时，先选择最能降低返工风险的流程。例如高影响创作且需求不清时，先确认需求，再进入章节、大纲、人设或审稿流程。""",
    INLINE_SKILL_GUIDANCE_PROMPT,
)
...
class AgentCore:
    def __init__(
        ...
        self.base_system_prompt = (
            ...
            else BASE_SYSTEM_PROMPT_WITH_INLINE_SKILLS
        )
        self.event_bus = EventBus()
        self.context_compressor = ContextCompressor()
        self.skill_loader = skill_loader or SkillLoader()
        self.skill_catalog_prompt = (
            self.skill_loader.build_catalog_prompt()
            if self.skills_enabled
            else ""
        )
    ...
    def _inject_base_system_prompt(self, messages: list[dict]):
        if any(
            msg.get("role") == "system"
            and msg.get("content") == self.base_system_prompt
            for msg in messages
        ):
            return messages
        ...
        while insert_at < len(prepared) and prepared[insert_at].get("role") == "system":
            insert_at += 1
        prepared.insert(insert_at, {"role": "system", "content": self.base_system_prompt})
        return prepared
