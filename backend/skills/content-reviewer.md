---
name: content-reviewer
description: 审查章节质量并给出分级问题清单和可执行修改建议
triggers:
  - 用户要求审查、审稿、审读、review、找问题、提出修改建议或润色建议时。
  - 用户希望检查章节的情节连贯、人物动机、节奏、文风、伏笔或逻辑漏洞时。
  - 用户提供或指定章节正文，需要输出分级问题清单和可执行修改建议时。
allowed_tools:
  - create_sub_agent
  - read_file
  - write_file
  - edit_file
  - list_files
  - search_files
priority: 30
---
# 内容审查

找出影响阅读、连贯性和题材氛围的问题，并给出可执行修改方向。此 skill 同时服务主 Agent 的任务编排和子 Agent 的审稿执行。

## 适用场景

- 用户要求审查、审稿、审读、找问题、修改建议或润色建议。
- 用户给出章节编号，需要读取后审查。
- 不适用于从零写章节；如果用户要求直接改写整章，可先审查，再由任务单明确是否改写和保存。

## 角色边界

- 主 Agent：只确认审查对象、关注点和是否需要保存，然后调用 `create_sub_agent` 委派；不要直接输出完整审稿意见、改写稿或调用保存工具。
- 子 Agent：执行任务单，读取章节、完成审查、按要求保存审稿意见或改写稿；不能向用户提问或等待输入，信息不足时把缺口和建议下一步写回执行结果。
- 主 Agent 委派时，`task` 必须写清：章节路径或正文来源、审查重点、是否读取前后文、是否需要改写、是否保存、目标路径、覆盖是否已确认、验收标准。
- 如果用户只说“帮我看看”但没有章节或文件路径，先用 requirement-confirmer 或简短追问确认对象。

## 上下文读取

- 正式审稿、改写或保存审稿意见前，先按 progress-summarizer 读取小说根目录的 `progress.md`，确认当前进度、最近变更和审查重点。
- 如果 `progress.md` 不存在，只做轻量目录检查，不要逐个读取所有文件；审稿意见、改写稿或保存结果生成后必须更新进度总结。
- 主 Agent 委派任务单应写清 `progress.md` 路径；子 Agent 返回结果时说明是否已更新进度总结。
- 文件路径使用当前 sandbox 相对路径，不要加 `novels/` 前缀。
- 如果用户只给章节编号，先使用 `search_files` 或 `list_files` 查找 `{novel_slug}/chapters/{chapter_id}.md`，再用 `read_file` 读取章节。
- 如需对照前后文，使用 `list_files` 和 `read_file` 读取相邻章节 Markdown。
- 不要在未读取章节内容时假装已经审查。

## 工作流程

1. 明确审查对象：章节编号、用户关注点、是否需要对照前后文。
2. 读取章节正文；读取失败时停止审查并说明缺少内容。
3. 检查情节连贯性、人物动机、信息矛盾、节奏、文风一致性、题材氛围。
4. 如果设定包含克苏鲁、悬疑、修仙、都市或科幻等类型元素，只检查对应氛围是否成立，不要强行套用不存在的题材标准。
5. 按严重程度输出问题：致命、重要、一般、可选优化。
6. 每条建议都要包含位置、问题、原因和修改方向。
7. 只有任务单或用户明确要求保存审稿意见、保存改写版时，才调用 `write_file` 或 `edit_file`。
8. 修改或生成新内容后，更新 `progress.md`，记录审稿状态、影响文件、待确认问题和下一步。

## 保存规则

- 保存审稿意见时只使用 `write_file` 或 `edit_file`，不要使用领域保存工具。
- 审稿意见默认写入 `{novel_slug}/reviews/{chapter_id}_review.md`，这是当前 sandbox 相对路径，不要加 `novels/` 前缀。
- 保存改写章节时写入 `{novel_slug}/chapters/{chapter_id}.md`，覆盖前必须确认。
- 文件类型必须是 Markdown，扩展名必须是 `.md`。
- `content` 必须是完整 Markdown 文件内容；审稿意见和改写正文不要混写到错误文件。
- 如果当前任务没有完整审稿意见或改写正文，先完成内容；无法完成时返回缺口，不要空保存。
- 不要擅自覆盖原章节；覆盖前要确认用户要保存改写版。

## Markdown 固定格式

```markdown
---
type: review
chapter_id: {chapter_id}
status: draft
---

# 审稿：{chapter_title}

## 总体判断

## 问题清单

### 致命

### 重要

### 一般

### 可选优化

## 修改方向
```

## 输出要求

- 没有明显问题时，说明通过，并补充一两条可选优化建议。
- 有问题时，避免泛泛评价，给出能直接执行的修改意见。
- 问题格式优先使用：严重程度、位置、问题、原因、修改方向。
- 审查结论先说总体判断，再列问题。

## 质量标准

- 问题必须落到具体位置或具体段落功能。
- 修改方向要能指导下一步改写，不要只说“加强描写”。
- 评价要匹配题材，不要把所有小说都按同一种文学标准审。

## 常见误区

- 主 Agent 读取本 skill 后直接输出完整审稿意见。
- 未读取章节正文就输出审查结论。
- 只给笼统评价，没有位置和修改方向。
- 把个人口味当成硬性问题。
- 用户只要求审查时擅自改写并保存。
- 保存时把审稿意见当成章节 `content`。
