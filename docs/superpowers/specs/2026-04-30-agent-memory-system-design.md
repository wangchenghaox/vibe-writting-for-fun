...
所有查询和写入必须在服务层强制注入命名空间。LLM 不能自行指定或覆盖 `user_id`，默认也不能查询其他 agent 的记忆。
...
- 校验 `memory_type`、`layer`、`importance`、`status`
- 处理标签 JSON 序列化
- 控制查询 limit
- 统一错误返回
- 为工具层提供简单接口
...
```text
remember_memory(content, memory_type="note", tags="", importance=3)
search_memory(query="", memory_type=None, tags="", limit=5)
list_memories(memory_type=None, tags="", limit=10)
...
@tool(
    "remember_memory",
    "记录一条长期记忆",
    context_params=["user_id", "novel_id", "agent_name"],
)
def remember_memory(
    content: str,
...
- 必选：`user_id`、`novel_id`、`agent_name`、`status = active`
- 可选：`memory_type`
- 可选：标签过滤
...
- 不同 `user_id` 的记忆互不可见。
- 不同 `novel_id` 的记忆互不可见。
- 不同 `agent_name` 的记忆互不可见。
- `archive_memory` 只能影响当前命名空间。
- `search_memory` 支持关键词、类型、标签、limit。

Agent 集成测试：

- `AgentCore` 调用工具时能注入 `user_id`、`novel_id`、`agent_name`。
...
- 增加记忆工具。
- Web 入口注入 `user_id + novel_id + agent_name`。
- AgentCore 写 Raw Log。
- 补充单元和集成测试。
...
- Agent 可以查询当前用户、当前小说、当前 agent 的 active 记忆。
- 不同用户、小说、agent 的记忆不会串。
