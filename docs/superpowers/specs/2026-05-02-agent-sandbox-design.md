...
系统需要区分两种角色，避免把工具权限和长期记忆命名空间混在一起。

权限角色来自 `AgentCore.tool_context["agent_role"]`，只决定工具权限。系统内置两个权限角色：
...
如果缺省，`agent_role` 默认为 `main`，保证旧入口默认按更保守权限运行。

业务角色来自 `AgentCore.tool_context["agent_name"]`，用于记忆系统命名空间和审计展示。主 agent 固定使用：
...
```text
agent_role = subagent
agent_name = writer | reviewer | worldbuilder | planner | editor | ...
```

`agent_name` 是稳定业务角色名，不是运行时实例 ID。配合既有记忆系统，长期记忆命名空间仍然是：

```text
user_id + novel_id + agent_name
```

因此 `writer`、`reviewer`、`worldbuilder` 等不同角色的 agent-scope 记忆不会混淆。同一小说下多次创建的 `writer` sub-agent 会共享 `writer` 的长期记忆；多次创建的 `reviewer` sub-agent 会共享 `reviewer` 的长期记忆。需要跨角色共享的小说正典、世界观规则和剧情事实应写入 `scope = novel`，而不是写入某个角色私有的 `scope = agent`。

运行时实例仍使用 `agent_instance_id` 追踪一次具体执行：

```text
agent_name = writer
agent_instance_id = subagent_writer_3
```

`agent_instance_id` 不参与长期记忆隔离，只用于 Raw Log、调试和事件追踪。
...
```text
writer       章节、正文、大纲落地
reviewer     内容审查、结构审稿、问题清单
worldbuilder 世界观、设定、规则整理
planner      创作计划、章节规划、任务拆分
editor       润色、改写、统一风格
```

角色列表是默认约定，不是硬编码上限。`create_subagent()` 可以接受自定义角色名，但必须做格式校验和规范化：

- 只允许小写字母、数字、下划线和短横线。
- 长度限制为 1 到 40 个字符。
- 拒绝 `main` 作为 sub-agent 业务角色，避免和主 agent 记忆混淆。
- 返回结果中同时展示稳定 `agent_name` 和运行时 `agent_instance_id`。

业务角色决定 memory namespace。主 agent 创建 sub-agent 时必须显式指定角色；如果用户没有指定，主 agent 应根据任务选择最贴切的默认角色，例如写章节用 `writer`，审稿用 `reviewer`。如果判断不清，主 agent 应先向用户确认，而不是创建泛化的 `subagent` 角色。
...
- `create_subagent()` 必须校验业务角色名，不能创建 `agent_name=main` 的 sub-agent。
...
create_subagent(role: str, instructions: str = "") -> str
run_subagent(subagent_id: str, task: str) -> str
```

这些工具通过隐藏上下文获取当前 `SandboxManager`、`SubAgentManager`、provider、session、sandbox 信息和 memory recorder factory。`create_sandbox()` 只允许在受控 novels 根目录内创建新的小说 sandbox，不能接收任意 root path。`switch_sandbox()` 只允许切换到当前用户可访问的小说 sandbox。`create_subagent()` 创建的 sub-agent 继承当前 active sandbox，权限角色强制为 `agent_role=subagent`，业务角色写入 `agent_name=<role>`，并生成新的 `agent_instance_id`。

`create_subagent()` 必须把规范化后的 `role` 同时传给 sub-agent 的 `tool_context`、`session.context` 和 `MemoryEventRecorder(agent_name=role, agent_instance_id=...)`。这样工具执行、长期记忆、Raw Log 和 WebSocket 事件使用同一套身份。
...
```text
agent_role = subagent
agent_name = 规范化后的业务角色名，例如 writer 或 reviewer
agent_instance_id = subagent_{agent_name}_{index}
```

4. sub-agent 的记忆工具使用 `user_id + novel_id + agent_name` 自动隔离角色记忆。
5. sub-agent 执行任务并返回结果。
6. 主 agent 把结果总结给用户。
...
- 创建 sub-agent 时角色非法：返回 `操作被拒绝: 无效 sub-agent 角色 {role}`。
- 创建 sub-agent 时角色为 main：返回 `操作被拒绝: sub-agent 不能使用 main 作为角色名`。
...
- `test_subagent_manager.py`
  ...
  - `create_subagent(role="writer")` 设置 `agent_name=writer`，并生成 `agent_instance_id=subagent_writer_{index}`。
  - `create_subagent(role="reviewer")` 设置 `agent_name=reviewer`，不复用 writer 的 agent-scope 记忆命名空间。
  - sub-agent 的 `tool_context`、`session.context` 和 `MemoryEventRecorder` 使用同一个规范化业务角色。
  - 创建 role 为 `main` 或包含路径字符的 sub-agent 会被拒绝。
  - 切换 active sandbox 后，旧 sub-agent 不能在新 sandbox 下继续执行。
  - sub-agent 仍保留 memory recorder 的运行时身份。

- `test_memory_tools.py`
  - `writer` 写入的 `scope=agent` 记忆不会被 `reviewer` 查询到。
  - `reviewer` 写入的 `scope=agent` 记忆不会被 `writer` 查询到。
  - `scope=novel` 记忆对同一 `user_id + novel_id` 下的 writer 和 reviewer 都可见。
  - 切换小说 sandbox 后，同名 `writer` sub-agent 不能读取另一部小说的 writer 记忆。
...
- sub-agent 必须有稳定业务角色名，例如 `writer` 或 `reviewer`；不同业务角色的 agent-scope 记忆互不可见。
- 同一 `user_id + novel_id + agent_name` 下重复创建的同角色 sub-agent 共享该角色长期记忆。
- `scope=novel` 记忆可在同一小说下跨业务角色共享，用于正典、世界观和全局剧情事实。
