...
- 主 agent 负责和用户交互、理解需求、制定计划，并通过受控编排能力管理 sandbox、创建并调度 sub-agent。默认 sandbox 由 Web/CLI 入口在创建主 agent 前建立，避免主 agent 在没有 sandbox 的状态下读取文件。
- 主 agent 只能通过工具读取和检索 sandbox 内文件，不能创建、写入、编辑、删除或重命名任何文件。
...
- 编排工具：`create_sandbox`、`create_subagent`、`run_subagent`

主 agent 不可见也不可执行的工具类型：

- 通用文件写入：`write_file`、`edit_file`、`delete_file`、`rename_file`
- 领域落盘：`create_novel`、`save_novel_document`
...
sub-agent 不可见编排工具：`create_sandbox`、`create_subagent`、`run_subagent`。第一版由主 agent 统一调度。
...
`SandboxManager` 不接受来自 LLM 的任意本地路径作为根目录。第一版只允许从受信任入口传入的当前小说目录，或在已配置的 sandbox base 目录下创建子 sandbox。
...
```text
create_sandbox(purpose: str = "") -> str
create_subagent(name: str, instructions: str = "") -> str
run_subagent(subagent_id: str, task: str) -> str
```

这些工具通过隐藏上下文获取当前 `SubAgentManager`、provider、session、sandbox 信息和 memory recorder factory。`create_sandbox()` 只允许创建当前会话 sandbox 的受控子目录，不能接收任意 root path。`create_subagent()` 创建的 sub-agent 继承当前 sandbox，但角色强制为 `subagent`，并覆盖 `agent_name` 和 `agent_instance_id`。
...
- `create_novel()`、`save_novel_document()` 标注为 `domain_write`，只有 sub-agent 可见。
...
5. 主 agent 读取资料、制定计划，需要写入时调用 `create_subagent()` 和 `run_subagent()`。
6. sub-agent 在同一 sandbox 内写入章节、大纲、审稿文件。
7. 前端详情页继续从同一小说目录读取章节文件。

### 7.2 CLI 会话

1. CLI 根据 `CURRENT_NOVEL_ID` 或 `default` 创建默认 sandbox。
2. 主 agent 获得只读文件工具和编排工具。
3. 写入任务通过 sub-agent 完成。
4. CLI 的 `/load` 等命令仍可更新主 agent 的 `novel_id`，同时必须刷新 sandbox context。
...
- `test_tool_policy.py`
  ...
  - main agent 伪造执行写入工具时被 `execute_tool()` 拒绝。
...
- 主 agent 在普通聊天中只能看到读取、检索、记忆、外部读取和编排工具。
